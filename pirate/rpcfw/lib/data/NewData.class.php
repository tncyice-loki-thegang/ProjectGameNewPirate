 ...
 * $Id: NewData.class.php 38103 2013-02-05 03:39:39Z HaopingBai $
 *
 **************************************************************************/

/**
 * @file $HeadURL: svn://192.168.1.80:3698/C/tags/pirate/rpcfw/rpcfw_1-0-21-57/lib/data/NewData.class.php $
 * @author $Author: HaopingBai $(hoping@babeltime.com)
 * @date $Date: 2013-02-05 11:39:39 +0800 (二, 2013-02-05) $
...
class CData
{
	...
	public function __construct($batchData = null)
	{
		...
		{
			self::$QUERY_CACHE = new LocalQueryCache ();
		}
		$this->reset ();
	}
	...
	public function select($arrSelect)
	{
		...
		{
			$this->reset ();
			Logger::fatal ( 'empty select field found' );
			throw new Exception ( 'inter' );
		}
		$this->command = 'select';
	...
	public function query($async = false)
	{
		...
		else
		{
			if (! self::$proxy)
			{
				self::$proxy = new PHPProxy ( $this->serviceName );
			}
			if (! empty ( $this->db ))
			{
			...
			$moreResult = true;
			if (! empty ( self::$QUERY_CACHE ))
			{
				Logger::debug ( "get data from cache now" );
				$arrCache = self::$QUERY_CACHE->beforeQuery ( $arrData, $moreResult );
			}

			if ($moreResult)
			{
				Logger::debug ( "more data needed from dataproxy" );
				$arrRet = self::$proxy->query ( $arrData );

				if (! empty ( self::$QUERY_CACHE ))
				{
					self::$QUERY_CACHE->afterQuery ( $arrData, $arrRet );
					if (! empty ( $arrCache ))
					{
