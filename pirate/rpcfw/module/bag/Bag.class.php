...
class Bag implements IBag
{
	...
	public function useItem($gid, $item_id, $item_num, $item_choose = FALSE)
	{
		...
		if ( !empty($useInfo) )
		{
			...
			//使用得到声望
			if ( isset($useInfo[ItemDef::ITEM_ATTR_NAME_USE_PRESTIGE]) )
			{
				if ( $user->addPrestige($useInfo[ItemDef::ITEM_ATTR_NAME_USE_PRESTIGE] * $item_num) == FALSE )
				{
			...
			{				
				...
				$info = AppleFactoryLogic::getInfo($this->m_uid);
				$curExp = $info['apple_experience'];
				AppleFactoryLogic::updateExpKernel($this->m_uid, $curExp+$retExp, NULL);
			}
			
			if ( isset($useInfo[ItemDef::ITEM_ATTR_NAME_USE_DEMON_KERNEL]) )
			{				
				$retKernel = $useInfo[ItemDef::ITEM_ATTR_NAME_USE_DEMON_KERNEL];
				$info = AppleFactoryLogic::getInfo($this->m_uid);
				$curKernel = $info['demon_kernel'];
				AppleFactoryLogic::updateExpKernel($this->m_uid, NULL, $curKernel+$retKernel);
			}			
			...
			{
				HorseDecorationLogic::addDecorationId($this->m_uid, $useInfo[ItemDef::ITEM_ATTR_NAME_USE_DECORATION]); 
			}
			...
			{
				HakiLogic::updateHakiInfo($this->m_uid, $useInfo[ItemDef::ITEM_ATTR_NAME_USE_DOMINEER]);
			}
			
			if ( isset($useInfo[ItemDef::ITEM_ATTR_NAME_USE_ELEMENT]) )
			{
				Jewelry::addEnergyElement($this->m_uid, 0, $useInfo[ItemDef::ITEM_ATTR_NAME_USE_ELEMENT]);
			}
		}
