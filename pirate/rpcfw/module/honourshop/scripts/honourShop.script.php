...
$name = array (
	'id' => $ZERO, 								// 表ID
	'itemTempLateName' => ++$ZERO,				// 可兑换名字
	'itemTempLateId' => ++$ZERO,				// 可兑换物品ID
	'itemTempLateNum' => ++$ZERO,				// 可兑换物品数量
	'consumeHonour' => ++$ZERO,					// 兑换需要物品所需荣誉
	'needPrestige' => ++$ZERO,					// 兑换需要物品所需声望
	'needLevel' => ++$ZERO,						// 兑换需要物品所需等级
	'exItemTimes' => ++$ZERO					// 物品可以兑换的次数
);

$file = fopen($argv[1].'/honor_shop.csv', 'r');
// 略过 前两行
$data = fgetcsv($file);
$data = fgetcsv($file);
...
while (TRUE)
{
	...
	{
		if($key == 'id' || $key == 'itemTempLateName')
		{
			continue;
		}
...
$file = fopen($argv[2].'/HONOURSHOP', 'w');
