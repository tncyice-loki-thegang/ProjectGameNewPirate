 ...
 * $Id: port.test.php 16423 2012-03-14 02:57:27Z HaopingBai $
 *
 **************************************************************************/

 /**
 * @file $HeadURL: svn://192.168.1.80:3698/C/tags/pirate/rpcfw/rpcfw_1-0-21-57/module/port/test/port.test.php $
...
function main()
{
	...
	//Test($proxy, 'attack', array(37700));
	$proxy->setClass ( 'port' );
	Test($proxy, 'enterPort', array(1));
	//Test($proxy, 'leavePort', array(1));
	//Test($proxy, 'moveInPort', array(1));
	Test($proxy, 'attackResource', array(1,1,1));
	Test($proxy, 'givenupResource', array(1,1,1));
	//Test($proxy, 'enterPortResource', array(1));
	//Test($proxy, 'leavePortResource', array());
	//Test($proxy, 'portBerthInfo', array(5,1));
	//Test($proxy, 'resourceInfo', array(1,1));
	Test($proxy, 'selfResourceInfo', array());
	//Test($proxy, 'selfBerthInfo', array());
}
