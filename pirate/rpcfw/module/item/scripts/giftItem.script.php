 ...
 * $Id: giftItem.script.php 11383 2011-12-26 06:27:12Z HaidongJia $
 *
 **************************************************************************/

 /**
 * @file $HeadURL: svn://192.168.1.80:3698/C/tags/pirate/rpcfw/rpcfw_1-0-21-57/module/item/scripts/giftItem.script.php $
...
ItemDef::ITEM_ATTR_NAME_GIFT_ITEM_NUM						=>		14,
ItemDef::ITEM_ATTR_NAME_USE_REQ								=>		35,
);
...
while ( TRUE )
{
	...
	if ( $array[ItemDef::ITEM_ATTR_NAME_GIFT_ITEM_NUM] <= 0 )
	{
		echo $array[ItemDef::ITEM_ATTR_NAME_ITEM_TEMPLATE_ID] . ' ' .
			ItemDef::ITEM_ATTR_NAME_GIFT_ITEM_NUM , "is null!\n";
		continue;
	}
	$items = array();
	for ( $i = 0; $i < $array[ItemDef::ITEM_ATTR_NAME_GIFT_ITEM_NUM]; $i++ )
	{
		$item_template_id = intval($data[$i*$attr_number+$name[ItemDef::ITEM_ATTR_NAME_GIFT_ITEM_NUM]+1]);
		$item_number = intval($data[$i*$attr_number+$name[ItemDef::ITEM_ATTR_NAME_GIFT_ITEM_NUM]+2]);
		$items[$item_template_id] = $item_number;
	}
	$array[ItemDef::ITEM_ATTR_NAME_USE_INFO][ItemDef::ITEM_ATTR_NAME_USE_ITEMS] = $items;
	...
	$gift[$array[ItemDef::ITEM_ATTR_NAME_ITEM_TEMPLATE_ID]] = $array;
}
fclose($file);

$file = fopen($argv[2], 'w');
fwrite($file, serialize($gift));
fclose($file);
