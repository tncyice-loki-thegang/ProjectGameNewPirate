 ...
 * $Id: Console.class.php 33080 2012-12-13 13:11:47Z wuqilin $
 *
 **********************************************************************************************************************/

/**
 * @file $HeadURL: svn://192.168.1.80:3698/C/tags/pirate/rpcfw/rpcfw_1-0-21-57/module/groupwar/test/Console.class.php $
 * @author $Author: wuqilin $(hoping@babeltime.com)
 * @date $Date: 2012-12-13 21:11:47 +0800 (四, 2012-12-13) $
 * @version $Revision: 33080 $
 * @brief
 *
 **/
	...
	public function hero($op, $num=0, $num2 = 0)
	{
		...
		switch ($op)
		{
			...
			case 'allHeroHp':
					...
					{
						$arr = array('htid'=>$attr['htid'], 'hid'=>$attr['hid'],
								'curHp'=>$attr['curHp'], 'exp'=>$attr['exp']);
						$heroObj = $user->getHeroObj($arr['hid']);
						$hp = $heroObj->getMaxHp()*$num;
						$hp = $hp>0?$hp :1;
						$heroObj->setHP($hp);
						$heroObj->update();
						$hpSum += $heroObj->getCurHp();
					}
					$user->update ();
					return $hpSum;
					break;
