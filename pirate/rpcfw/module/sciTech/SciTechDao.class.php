...
class SciTechDao
{
     ...
     ******************************************************************************************************************/
	// 定义表的名称
	private static $tblST = 't_sci_tech';
	 ...
	 * @return 返回相应信息
	 */
	public static function getStInfo($uid)
	{
		// 优先检查缓冲区数据
	 ...
	 */
	public static function updStInfo($uid, $set)
	{
		Logger::debug("updStInfo called, buffer is %s, set is %s.", self::$buffer, $set);
		// 缓存不为空，则进行比较
		if (!empty(self::$buffer[$uid]))
		{
			// 判断，如果什么都没改动，那么直接返回
			...
			{
				Logger::debug("Upd st array diff ret is same.");
				return $set;
			}
		}
	 ...
	 * @param string $uid						用户ID
	 */
	public static function addNewStInfo($uid)
	{
    	// 设置活动字段
    	$vaArr = array('st_id_lv' => array());
		// 设置属性
		$arr = array('uid' => $uid,
					 'cd_time' => 0,
					 'cd_status' => 'F',
		...
		$data = new CData();
		$arrRet = $data->insertInto(self::$tblST)
		               ->values($arr)->query();
